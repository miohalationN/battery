# 电池档案（BatteryArchive · BA）

macOS 菜单栏电池与功耗记录应用。产品简称 **BA**，中文名 **电池档案**，副标题
「电池健康与功耗记录」。内部可执行文件、数据目录与 GitHub Artifact
...
3. 默认零权限运行；如需 CPU/GPU 内存分项功耗，在「功耗分析 → 高级采样」手动开启
   （安装时弹一次管理员密码）；设置页「采样诊断」可移除后台服务（同样需要管理员授权）
